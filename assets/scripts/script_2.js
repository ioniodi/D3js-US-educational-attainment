...
d3.json("assets/data/us.json", function(data) {
    ...
    function display(d) {
        ...
        document.forms[0].addEventListener("change", function() {
            ...
            .sum(function (d) {
                if (treeSumSortType == "number") {
                    color = d3.scaleLinear().domain([0, 1/4*5000000, 2/4*5000000, 3/4*5000000, 5000000]).range(["#dedea6", "#0a3b0d", "#f20361", "#f2db84"]);
                    return d["Total College"];
                } else if (treeSumSortType == "percent") {
                    color = d3.scaleLinear().domain([0, 1/4*50, 2/4*50, 3/4*50, 50]).range(["#73c3bf", "#a1d8c8", "#f20361", "#f2db84"]);
                    return d["Percent College"];
